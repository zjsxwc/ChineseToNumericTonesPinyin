...
const (
	...
	Tone                               // 带声调的拼音 例如：guó
	ToneUseNumeric                               // 带数字声调的拼音 例如：guo2
...
func GetToneNumeric(tone rune) string {
	n, ok := numericTonesMap[tone]
		if !ok {
			return ""
		}
...
func getPinyin(hanzi rune, mode Mode) (string, error) {
	...
	case ToneUseNumeric:
		return getToneUseNumeric(hanzi), nil
...
func getToneUseNumeric(hanzi rune) string {
	tone := pinyinMap[hanzi]
	numeric := ""
	for _, t := range tone {
		numeric = GetToneNumeric(t);
		if numeric != "" {
			break
		}
	}
	return getDefault(hanzi)+numeric
}
